...
def get_csv_data(start_date : str, benchmark : str, asset : str) -> pd.DataFrame :
    INTERVAL = "1d"
    asset_data = yf.Ticker(asset).history(start=start_date, interval=INTERVAL, auto_adjust=True)
    benchmark_data = yf.Ticker(benchmark).history(start=start_date, interval=INTERVAL, auto_adjust=True)
